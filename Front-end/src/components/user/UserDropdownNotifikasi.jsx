...
export default function UserDropdownNotifikasi({ selectedOption }) {
  ...
  return (
    <div className="w-1/4 flex flex-col relative" ref={dropdownRef}>
      <button
        id="selectBtn"
        className="bg-[#1E1633] flex justify-between items-center cursor-pointer border border-[rgba(255,236,120,0.5)] py-2 px-4 rounded-sm"
        onClick={handleToggle}
      >
        <div className="flex items-center gap-x-3">
          <CiFilter className="h-fit w-6.5 text-[#FEF8FD]" />
      ...
      >
        {["Semua Notifikasi", "Belum Dibaca", "Sudah Dibaca"].map((item) => (
          <li
            key={item}
